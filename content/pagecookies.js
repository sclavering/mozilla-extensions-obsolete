function tbextCookie(name,value,isDomain,host,rawHost,path,isSecure,expires,status,policy) {
  this.name = name;
  this.value = value;
...
var tbextPageCookiesMenu = {
  cookieManager: null,
  
  bundle: null,
  
  getBundle: function() {
    if(!this.bundle) this.bundle = document.getElementById("tbext-cookieinfo-bundle");
    return this.bundle;
  },
  ...
  getCookies: function() {
    ...
    while(enumerator.hasMoreElements()) {
      ...
      var cookieobj = new tbextCookie(cookie.name, cookie.value, cookie.isDomain, rawhost, host,
  ...
  showMenu: function(menu, evt) {
    ...
    for(var host in this.cookiesByHost) {
      ...
      popup.addEventListener("command", function(e){tbextPageCookiesMenu.commanded(e);}, false);
      ...
      for(var i = 0; i < cookies.length; i++) {
        ...
        item.setAttribute("tooltip","tbext-pagecookies-tooltip");
...
var tbextCookieInfo = {
  ...
  showInfo: function(cookie) {
    ...
    for(var i = 0; i < fields.length; i++) {
      var field = document.getElementById("tbext-cookieinfo-"+fields[i]);
